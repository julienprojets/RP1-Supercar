...
<!-- End Sidekick -->
    ...
    <div class="full-page-section">
        <h1 class="scroll-text">Défilez vers le bas et rapprochez-vous de votre demande d'essai</h1>
    </div>
    

    <div class="container form-section">
        <h2 class="text-center">VOTRE DEMANDE D'ESSAI N'A JAMAIS ÉTÉ AUSSI PROCHE!</h2>
        <form id="testDriveForm" action="submit_request-fr.php" method="POST">
            <div class="row mb-3">
                <div class="col-md-6">
            ...
            <div class="row mb-3">
                <div class="col-md-12">
                    <label for="comment">Commentaires/Demandes</label>
                    <textarea id="comment" class="form-control1" name="comments" rows="4" placeholder="Entrez votre message..." style="border: 2px solid black;"></textarea>
                </div>
            </div>
            <div class="mb-3">
                <select class="form-control1" required style="border: 2px solid black;" name="brand" id="brandSelect">
                    <option selected disabled>-Veuillez choisir la marque souhaitée-</option>
            ...
            <div class="d-flex justify-content-center">
                <button type="submit" class="btn btn-dark w-50">ENVOYER</button>
            </div>
        </form>
    </div>
...
<script>
    ...
    brandSelect.addEventListener('change', function () {
        ...
        modelSelect.innerHTML = '<option selected disabled>-Veuillez choisir la marque souhaitée-</option>';
...
document.getElementById("testDriveForm").addEventListener("submit", function(event) {
    ...
    .then(data => {
        ...
        if (data.status === "success") {
            showPopup("✅ La demande a été envoyée!");
            document.getElementById("testDriveForm").reset(); // Clear form
        } else {
            showPopup("❌ Veuillez Réessayer: " + data.message);
        }
    })
    .catch(error => {
        console.error("Fetch error:", error);
        showPopup("⚠️ Veuillez réessayer plus tard.");
    });
});

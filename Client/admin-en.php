...
function fetchContent($table, $section) {
    ...
    if ($result->num_rows > 0) {
        ...
        return "Text not available"; // Default text if not found
    }
}
...
function fetchImage($table, $section) {
    ...
    if ($result->num_rows > 0) {
        ...
        return "Image not available"; // Default text if not found
    }
}
...
function fetchMake($table, $section) {
    ...
    if ($result->num_rows > 0) {
        ...
        return "Car Manufacturer not available"; // Default text if not found
    }
}
...
function fetchModel($table, $section) {
    ...
    if ($result->num_rows > 0) {
        ...
        return "Model not available"; // Default text if not found
    }
}
...
function fetchType($table, $section) {
    ...
    if ($result->num_rows > 0) {
        ...
        return "Type not available"; // Default text if not found
    }
}
...
function fetchPrice($table, $section) {
    ...
    if ($result->num_rows > 0) {
        ...
        return "Price not available"; // Default text if not found
    }
}
...
// Function to fetch Overview-en dynamically
function fetchOverviewEn($table, $section) {
    global $conn;
    
    // Ensure table name is safe (avoid SQL injection)
    $table = preg_replace('/[^a-zA-Z0-9_]/', '', $table);
    
    $sql = "SELECT overview_en FROM $table WHERE car_num = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $section);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        return $result->fetch_assoc()['overview_en'];
    } else {
        return "Text not available"; // Default text if not found
    }
}
// Function to fetch Specs-en dynamically
function fetchSpecsEn($table, $section) {
    global $conn;
    
    // Ensure table name is safe (avoid SQL injection)
    $table = preg_replace('/[^a-zA-Z0-9_]/', '', $table);
    
    $sql = "SELECT specs_en FROM $table WHERE car_num = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $section);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        return $result->fetch_assoc()['specs_en'];
    } else {
        return "Specifications not available"; // Default text if not found
    }
}
?>
